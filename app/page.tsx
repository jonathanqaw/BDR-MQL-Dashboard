...
const HISTORICAL_STATUSES: Record<string,Status> = {
  ...
  'jpmorganchase@historical':     'booked',
  'pex@historical':               'booked',
  'productleague@historical':     'booked',
}
...
export default function Dashboard() {
  ...
  const [mtgCustomTo,setMtgCustomTo]=useState('')
  ...
  const hasActivityInRange=(l:AppLead,start:Date,end:Date):boolean=>{
    // Primary: lead's creation/received date determines which period it belongs to
    const creationDate=l.receivedAt||l.date
    if(creationDate){
      const dt=new Date(creationDate)
      if(!isNaN(dt.getTime())&&dt>=start&&dt<=end) return true
    }
    // Secondary: if no creation date, check activity dates as fallback (historical leads)
    if(!creationDate){
      ...
      return activityDates.some(d=>{const dt=new Date(d as string);return !isNaN(dt.getTime())&&dt>=start&&dt<=end})
    }
    return false
  }
      ...
      <main style={{flex:1,padding:'30px 34px 60px',overflowX:'auto',overflowY:'auto',minWidth:0,height:'100vh'}}>
        ...
        {view==='pipeline'&&(<>
          ...
          {(()=>{
                ...
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>
                    {tableLeads.length===0
                      ...
                      :tableLeads.map(lead=>{
                        const sourcePill=<span style={{fontSize:8,fontWeight:700,padding:'1px 5px',borderRadius:3,marginLeft:5,verticalAlign:'middle',background:isOutbound(lead)?'rgba(232,121,249,0.15)':'rgba(96,212,244,0.15)',color:isOutbound(lead)?'#e879f9':'#60d4f4'}}>{isOutbound(lead)?'Outbound':'Inbound'}</span>
            ...
            const olderLeads2=tableLeads.filter(l=>getKey2(l)!==tKey)
            const dayMap2=new Map<string,AppLead[]>()
            olderLeads2.forEach(l=>{const k=getKey2(l);if(!dayMap2.has(k))dayMap2.set(k,[]);dayMap2.get(k)!.push(l)})
            const dayGroups2=Array.from(dayMap2.entries()).sort((a,b)=>b[0].localeCompare(a[0]))
            const fmtDay2=(isoDay:string)=>{const d=new Date(isoDay+'T12:00:00');if(isNaN(d.getTime()))return isoDay;const y=new Date();y.setDate(y.getDate()-1);const yKey=`${y.getFullYear()}-${String(y.getMonth()+1).padStart(2,'0')}-${String(y.getDate()).padStart(2,'0')}`;if(isoDay===yKey)return 'Yesterday';return d.toLocaleDateString('en-US',{weekday:'long',month:'short',day:'numeric',year:'numeric'})}
            return (<>
              {todayLeads2.length>0&&(<>
                <div style={{fontSize:10,fontWeight:700,color:C.green,textTransform:'uppercase',letterSpacing:'.06em',marginBottom:6}}>Today · {todayLeads2.length} leads</div>
                <div style={{background:C.surface,border:`1px solid ${C.border}`,borderRadius:10,overflow:'hidden',marginBottom:12}}>
                  <table style={{width:'100%',borderCollapse:'collapse'}}>
                    <thead><tr style={{background:C.surface2,borderBottom:`1px solid ${C.border}`}}><th style={{width:4,padding:0}}/>{['Account / Email','Domain / Source','SF','Date','Status','Quality'].map(h=>(<th key={h} style={{textAlign:'left',padding:'10px 14px',fontSize:10,fontWeight:700,color:C.text3,textTransform:'uppercase',letterSpacing:'.08em',whiteSpace:'nowrap'}}>{h}</th>))}</tr></thead>
                    <tbody>{todayLeads2.map(lead=><React.Fragment key={lead.email}>{renderRow(lead)}</React.Fragment>)}</tbody>
                  </table>
                </div>
              </>)}
        ...
        {view==='analytics'&&(<>
          ...
          {false&&(()=>{
            ...
            const range=getMtgRange(mtgSeg,mtgCustomFrom,mtgCustomTo)
            const BOOKED_SET=new Set<Status>(['booked','inprogress','closedwon'])
            const mtgInRange=(l:AppLead):boolean=>{
              const det2=details[l.email];if(!det2?.meetingDate)return false
              const md=new Date(det2.meetingDate);return md>=range.start&&md<=range.end
            }
            ...
            const rangeMs2=Math.max(1,range.end.getTime()-range.start.getTime())
            const prevRange={start:new Date(range.start.getTime()-rangeMs2),end:new Date(range.end.getTime()-rangeMs2)}
            const prevInRange=(l:AppLead):boolean=>{
              const det2=details[l.email];if(!det2?.meetingDate)return false
            ...
            const pNurture=prevLeads.filter(l=>(statuses[l.email]||'new')==='nurture').length
            const pLost=prevLeads.filter(l=>(statuses[l.email]||'new')==='lost').length
            const delta2=(a:number,b:number)=>{if(b===0)return a>0?'+∞':'—';const pct2=Math.round((a-b)/b*100);return pct2>0?`+${pct2}%`:pct2===0?'0%':`${pct2}%`}
            const dColor=(a:number,b:number)=>a>b?C.green:a<b?C.red:C.text3
            const segLabels:{[k:string]:string}={week:'This Week',month:'This Month',quarter:'This Quarter',year:'YTD',custom:'Custom'}
            const compLabels:{[k:string]:string}={week:'prev week',month:'prev month',quarter:'prev quarter',year:'prev year',custom:'prev range'}
            const metrics=[
              {label:'Meetings Booked',cur:booked,prev:pBooked,color:C.green},
              {label:'Meetings Held',cur:held,prev:pHeld,color:'#60d4f4'},
              {label:'SQL',cur:sql,prev:pSql,color:'#c084fc'},
            ...
            return (
              <div style={{...card,marginTop:24}}>
                <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:16,flexWrap:'wrap',gap:8}}>
                  <div style={{fontSize:11,fontWeight:700,color:C.text3,textTransform:'uppercase',letterSpacing:'.08em'}}>Meeting Outcomes</div>
                  <div style={{display:'flex',gap:5,alignItems:'center'}}>
                    {(['week','month','quarter','year','custom'] as const).map(s=>(
                      <button key={s} onClick={()=>setMtgSeg(s)} style={filterPill(mtgSeg===s)}>{segLabels[s]}</button>
                    ))}
                    <button onClick={()=>setMtgCompare(c=>!c)} style={{...filterPill(mtgCompare,C.amber),fontSize:10,marginLeft:4}}>
                      {mtgCompare?`✕ vs ${compLabels[mtgSeg]}`:`⇄ Compare`}
                    </button>
                  </div>
                </div>
                {mtgSeg==='custom'&&(
                ...
                <div style={{display:'grid',gridTemplateColumns:`repeat(${metrics.length},1fr)`,gap:10,marginBottom:mtgCompare?16:0}}>
                  {metrics.map(m=>(
                    <div key={m.label} style={{background:C.surface3,border:`1px solid ${C.border}`,borderRadius:8,padding:'10px 12px'}}>
                      <div style={{fontSize:9,fontWeight:700,color:C.text3,textTransform:'uppercase',letterSpacing:'.06em',marginBottom:6}}>{m.label}</div>
                      <div style={{fontSize:22,fontWeight:800,color:m.color}}>{m.cur}</div>
                      {mtgCompare&&<div style={{fontSize:10,color:dColor(m.cur,m.prev),marginTop:4,fontWeight:600}}>{delta2(m.cur,m.prev)} <span style={{color:C.text3,fontWeight:400}}>vs {m.prev}</span></div>}
                    </div>
                  ))}
                </div>
                {mtgCompare&&(
                  <table style={{width:'100%',borderCollapse:'collapse',fontSize:12}}>
                    <thead>
                      <tr style={{borderBottom:`2px solid ${C.border2}`}}>
                        <th style={{padding:'8px 10px',textAlign:'left',fontSize:10,fontWeight:700,color:C.text3,textTransform:'uppercase',letterSpacing:'.06em'}}>Metric</th>
                        <th style={{padding:'8px 10px',textAlign:'right',fontSize:10,fontWeight:700,color:C.purpleL,textTransform:'uppercase',letterSpacing:'.06em'}}>{segLabels[mtgSeg]}</th>
                        <th style={{padding:'8px 10px',textAlign:'right',fontSize:10,fontWeight:700,color:C.amber,textTransform:'uppercase',letterSpacing:'.06em'}}>{compLabels[mtgSeg]}</th>
                        <th style={{padding:'8px 10px',textAlign:'right',fontSize:10,fontWeight:700,color:C.text3,textTransform:'uppercase',letterSpacing:'.06em'}}>Delta</th>
                      </tr>
                    </thead>
                    <tbody>
                      {metrics.map(m=>(
                        <tr key={m.label} style={{borderBottom:`1px solid ${C.border}`}}>
                          ...
                          <td style={{padding:'8px 10px',textAlign:'right',fontWeight:700,color:C.purpleL}}>{m.cur}</td>
                          <td style={{padding:'8px 10px',textAlign:'right',fontWeight:700,color:C.amber}}>{m.prev}</td>
                          <td style={{padding:'8px 10px',textAlign:'right',fontWeight:700,color:dColor(m.cur,m.prev)}}>{delta2(m.cur,m.prev)}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
        ...
        {view==='revops_commissions'&&(()=>{
          ...
          const repData = reps.filter(r=>r.slackId).map(rep => {
            ...
            repLeads.forEach(l => {
              ...
              // One row per account per month. Both meeting ($150) and SQL ($620) commissions
              // are factored into the amount for accounts that have both.
              if (hasSql && det.sqlDate && hasMeetingDate) {
                ...
                if (mtgMonth !== sqlMonth) {
                  // Different months: meeting credit in meeting's month, SQL+meeting credit in SQL's month
                  events.push({...baseFields,meetingDate:det.meetingDate,sqlDate:null,isMeeting:true,isSql:false,amount:MEETING_BONUS})
                  events.push({...baseFields,meetingDate:null,sqlDate:det.sqlDate,isMeeting:false,isSql:true,amount:SQL_BONUS + MEETING_BONUS})
                } else {
                  // Same month: one row with both commissions combined
                  events.push({...baseFields,meetingDate:det.meetingDate,sqlDate:det.sqlDate,isMeeting:true,isSql:true,amount:SQL_BONUS + MEETING_BONUS})
                }
